...
| `hexaglue-spi` | What a backend implements: a manifest — who it is, which options it answers to, which family of ports it will write adapters for — and one pure function from what the run concluded to what it hands over: documents, Java types, and what it declined to write. Backends are ordered by their declared dependencies and isolated from one another, and none of them ever touches a disk |
