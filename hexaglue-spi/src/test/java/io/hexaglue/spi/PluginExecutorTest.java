...
class PluginExecutorTest {
    ...
    private record TestPlugin(PluginManifest manifest, Consumer<Sinks> behaviour) implements HexaGluePlugin {
        ...
        static TestPlugin generating(String id, String typeName) {
            return new TestPlugin(
                    PluginManifest.of(id),
                    sinks -> sinks.sources().emit(SourceFile.of("com.acme", typeName, "generated by " + id)));
        }
    ...
    class DocumentConflicts {
        ...
        @Test
        @DisplayName("and does the same for a type two backends both generate")
        void keepsTheFirstSource() {
            PluginRun run = run(List.of(TestPlugin.generating("a", "Order"), TestPlugin.generating("b", "Order")));

            assertThat(run.sources())
                    .singleElement()
                    .satisfies(source -> assertThat(source.content()).isEqualTo("generated by a"));
            assertThat(coded(run, SOURCE_CLAIMED_TWICE).orElseThrow().message())
                    .contains("com.acme.Order")
                    .contains("a")
                    .contains("b");
        }
    }
    ...
    @Nested
    @DisplayName("a backend that writes code")
    class WritingCode {

        @Test
        @DisplayName("hands its types over apart from the prose the run also produces")
        void handsItsTypesOverApartFromTheProse() {
            PluginRun run =
                    run(List.of(TestPlugin.generating("jpa", "OrderEntity"), TestPlugin.writing("doc", "a.md")));

            assertThat(run.sources()).extracting(SourceFile::qualifiedName).containsExactly("com.acme.OrderEntity");
            assertThat(run.documents()).extracting(Document::path).containsExactly("a.md");
        }

        @Test
        @DisplayName("and leaves nothing behind when it fails halfway through")
        void andLeavesNothingBehindWhenItFails() {
            HexaGluePlugin halfway = new TestPlugin(PluginManifest.of("jpa"), sinks -> {
                sinks.sources().emit(SourceFile.of("com.acme", "Half", "class Half {}"));
                throw new IllegalStateException("no");
            });

            PluginRun run = run(List.of(halfway));

            assertThat(run.sources()).isEmpty();
            assertThat(coded(run, PLUGIN_FAILED)).isPresent();
        }
    }

    @Nested
    @DisplayName("what a plugin is handed to emit with")
    class WhatAPluginEmitsWith {

        /**
         * The sinks are reachable, but a backend is meant to go through the contribution it was
         * handed: these three are the whole of what one writes with, so they are exercised the way
         * a plugin uses them rather than through the sinks underneath.
         */
        @Test
        @DisplayName("reaches all three channels without a plugin touching a sink itself")
        void reachesAllThreeChannels() {
            HexaGluePlugin backend = new HexaGluePlugin() {
                @Override
                public PluginManifest manifest() {
                    return PluginManifest.of("jpa");
                }

                @Override
                public void contribute(Contribution contribution) {
                    contribution.emit("notes.md", "what was generated");
                    contribution.emit(SourceFile.of("com.acme", "OrderEntity", "class OrderEntity {}"));
                    contribution.report(
                            Diagnostic.builder(IssueCode.of("HG-JPA-001"), DiagnosticSeverity.INFO, "skipped Cart")
                                    .build());
                }
            };

            PluginRun run = run(List.of(backend));

            assertThat(run.documents()).extracting(Document::path).containsExactly("notes.md");
            assertThat(run.sources()).extracting(SourceFile::qualifiedName).containsExactly("com.acme.OrderEntity");
            assertThat(coded(run, IssueCode.of("HG-JPA-001")).orElseThrow().message())
                    .isEqualTo("skipped Cart");
        }
    }

    @Nested
    @DisplayName("a backend that declines part of its work")
    class DecliningPartOfTheWork {

        private static final IssueCode TOO_UNSURE = IssueCode.of("HG-JPA-001");

        @Test
        @DisplayName("says so and goes on contributing the rest")
        void saysSoAndGoesOnContributing() {
            HexaGluePlugin cautious = new TestPlugin(PluginManifest.of("jpa"), sinks -> {
                sinks.diagnostics()
                        .report(Diagnostic.builder(TOO_UNSURE, DiagnosticSeverity.WARNING, "not sure enough about Cart")
                                .build());
                sinks.sources().emit(SourceFile.of("com.acme", "OrderEntity", "class OrderEntity {}"));
            });

            PluginRun run = run(List.of(cautious));

            assertThat(run.executed()).containsExactly("jpa");
            assertThat(run.sources()).hasSize(1);
            assertThat(coded(run, TOO_UNSURE).orElseThrow().message()).contains("Cart");
        }

        @Test
        @DisplayName("and what it reported dies with it when it then fails")
        void andWhatItReportedDiesWithItWhenItFails() {
            HexaGluePlugin doomed = new TestPlugin(PluginManifest.of("jpa"), sinks -> {
                sinks.diagnostics()
                        .report(Diagnostic.builder(TOO_UNSURE, DiagnosticSeverity.WARNING, "not sure enough")
                                .build());
                throw new IllegalStateException("no");
            });

            PluginRun run = run(List.of(doomed));

            assertThat(coded(run, TOO_UNSURE)).isEmpty();
            assertThat(coded(run, PLUGIN_FAILED)).isPresent();
        }
    }

    @Nested
    @DisplayName("the certainty a run demands before code is written from a verdict")
    class TheCertaintyARunDemands {

        private static Confidence askedOf(Confidence stated, Confidence reached) {
            ArchModel model = ArchModel.builder()
                    .addType(new ValueObject(
                            TypeId.of("com.acme.Money"),
                            TypeStructure.builder(TypeNature.RECORD).build(),
                            Classification.builder(
                                            ArchKind.VALUE_OBJECT,
                                            reached,
                                            Basis.INFERRED,
                                            ProofNode.fact("by fixture"))
                                    .build()))
                    .build();
            AtomicReference<Boolean> answer = new AtomicReference<>();
            HexaGluePlugin asking = new TestPlugin(PluginManifest.of("jpa"), sinks -> {});
            PluginExecutor.run(
                    List.of(new HexaGluePlugin() {
                        @Override
                        public PluginManifest manifest() {
                            return asking.manifest();
                        }

                        @Override
                        public void contribute(Contribution contribution) {
                            answer.set(contribution.isCertainEnough(
                                    contribution.model().types().get(0)));
                        }
                    }),
                    model,
                    List.of(),
                    ...
                    stated,
                    Map.of());
            return Boolean.TRUE.equals(answer.get()) ? reached : null;
        }

        @Test
        @DisplayName("accepts a verdict that reaches it")
        void acceptsAVerdictThatReachesIt() {
            assertThat(askedOf(Confidence.HIGH, Confidence.EXPLICIT)).isEqualTo(Confidence.EXPLICIT);
            assertThat(askedOf(Confidence.HIGH, Confidence.HIGH)).isEqualTo(Confidence.HIGH);
        }

        @Test
        @DisplayName("and refuses one that falls short, which is the way round a backend gets wrong")
        void andRefusesOneThatFallsShort() {
            assertThat(askedOf(Confidence.HIGH, Confidence.MEDIUM)).isNull();
            assertThat(askedOf(Confidence.HIGH, Confidence.LOW)).isNull();
        }

        @Test
        @DisplayName("and demands only what the run stated")
        void andDemandsOnlyWhatTheRunStated() {
            assertThat(askedOf(Confidence.LOW, Confidence.MEDIUM)).isEqualTo(Confidence.MEDIUM);
        }
    }
}
